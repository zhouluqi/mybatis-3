...
public class PooledDataSource implements DataSource {
    ...
    //获取连接时间，这是一个底层设置，如果获取连接花费了相当长的时间，
    ...
    // 但是这个重新尝试的次数不应该超过 poolMaximumIdleConnections 与
    // poolMaximumLocalBadConnectionTolerance 之和。 默认值：3
    ...
    //构造方法获取非池化的数据源对象---获取真正连接的逻辑，还是在 UnpooledDataSource 中实现。
    ...
    //计算  expectedConnectionTypeCode 的值 路径+用户名+密码的字符串hashcode值
        ...
        synchronized (state) {
            ...
            if (conn.isValid()) {
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    //创建新的连接对象，并添加空闲的连接集合当中
                    PooledConnection newConn = new PooledConnection(conn.getRealConnection(), this);
                    state.idleConnections.add(newConn);
                    newConn.setCreatedTimestamp(conn.getCreatedTimestamp());
                    newConn.setLastUsedTimestamp(conn.getLastUsedTimestamp());
                    // 设置原连接失效
                    // 为什么这里要创建新的 PooledConnection 对象呢？避免使用方还在使用 conn ，
    ...
    private PooledConnection popConnection(String username, String password) throws SQLException {
        ...
        int localBadConnectionCount = 0;      //记录获取到坏的连接次数
        ...
        while (conn == null) {
            synchronized (state) {
                ...
                if (!state.idleConnections.isEmpty()) {
                    ...
                    // 激活的连接数小于 poolMaximumActiveConnections，新new一个连接
